timesim<-function(pseq, treat,t_col, t_var,...)
{
  ...
  d<-prune_samples(sample_data(n_pseq)$Time==t_var,n_pseq)
  ...
  sim=with(data.frame(sample_data(d)),simper(simper.matrix,category))
  fout = tidy(capture.output(summary(sim)))
  

  
  
  fout %>% map(write.table,file= paste(t_var,"_Simper_summary.csv"),append= T, sep=',')
  
  cat("SIMPER analysis on",t_var,"data is completed. Results are written to the file Diet_simper_summary.csv\n")
}